-- SQL Testing Code for Data Quality Check

-- Set Unity Catalog schema
USE purgo_playground;

-- Setup: Temporary Table Creation
-- Create a temporary view for the test cases
...
SELECT COUNT(*) AS count_null_item_nbr FROM test_data WHERE item_nbr IS NULL;
...
SELECT COUNT(*) AS count_null_sellable_qty FROM test_data WHERE sellable_qty IS NULL;
...
-- Count records where 'prod_exp_dt' does not conform to valid dates in range
SELECT COUNT(*) AS count_invalid_prod_exp_dt FROM test_data 
...
WHERE prod_exp_dt NOT BETWEEN 19000101 AND 20991231 LIMIT 5;

-- Clean up temporary view after tests
